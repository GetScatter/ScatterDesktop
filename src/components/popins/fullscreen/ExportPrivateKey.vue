			...
			<section v-if="state === STATES.SELECT">
				...
				<section class="types">
					...
					<section class="type" @click="createQR">
						...
						<figure class="name">Paper Wallet</figure>
...
<script>
	...
	const {Wallet} = remote.getGlobal('appShared');
	...
	export default {
		...
		computed:{
			...
			keypair(){
				return Keypair.fromJson(this.popin.data.props.keypair);
			}
		},
		...
		methods:{
			...
			async init(){
				this.privateKey = await Wallet.getPrivateKey(this.keypair.id, this.keypair.enabledKey().blockchain);
			},
			getPublicKey(){
				return this.keypair.enabledKey().key;
			},
			async copyPrivateKey(){
				const prv = await this.privateKey;
			...
			async createQR(){
				this.qr = await QRService.createQR(this.privateKey);
			...
			screenshot(){
				...
				setTimeout(() => {
					let location = FileService.getFolderLocation();
					...
					remote.getCurrentWindow().capturePage(img => {
						FileService.saveFile(location, filename, img.toJPEG(99)).then(saved => {
							PopupService.push(Popup.snackbar(`QR Code saved!`, 'check'));
							ElectronHelpers.openLinkInBrowser(location, true);
							setTimeout(() => {
