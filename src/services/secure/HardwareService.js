...
const isLedgerConnected = () => StoreService.get().state.hardware.hasOwnProperty(EXT_WALLET_TYPES.LEDGER);
const hardwareKeypairs = (type) => StoreService.get().getters.keypairs.filter(x => !!x.external && x.external.type === type);
...
export default class HardwareService {
	...
	static async addLedger(device){
		...
		await StoreService.get().dispatch(Actions.SET_HARDWARE, {name:EXT_WALLET_TYPES.LEDGER, transport:openedTransport});
	...
	static async removeLedger(device){
		if(isLedgerConnected()){
			if(StoreService.get().state.hardware.hasOwnProperty(EXT_WALLET_TYPES.LEDGER))
				await StoreService.get().state.hardware[EXT_WALLET_TYPES.LEDGER].disconnect();

			await StoreService.get().dispatch(Actions.REMOVE_HARDWARE, EXT_WALLET_TYPES.LEDGER);
